...
import { Plus, X, Trash2, Edit2, CheckCircle, Archive, RefreshCw, Save, Calendar, User, Wrench } from "lucide-react";

const ITTasksManager = ({ getToken, showMessage }) => {
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        <div style={styles.cardHeader}>
          <h3 style={{ margin: 0, fontSize: 16 }}>🛠️ Задачи IT-отдела</h3>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={fetchTasks} style={styles.buttonPrimary}>
              <RefreshCw size={14} /> Обновить
            </button>
            <button onClick={() => setShowArchived(!showArchived)} style={styles.buttonPrimary}>
              {showArchived ? "📋 Активные" : "🗄️ Архив"}
            </button>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={styles.taskMeta}>
                    {task.assigned_to && <span><User size={12} /> Кому: {task.assigned_to}</span>}
                    {task.executor && <span><Wrench size={12} /> Исполнитель: {task.executor}</span>}
                    {task.due_date && <span><Calendar size={12} /> Срок: {task.due_date}</span>}
      ...
      {showTaskForm && (
        <div style={styles.modal} onClick={() => setShowTaskForm(false)}>
